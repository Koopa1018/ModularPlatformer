...
namespace Clouds.Platformer.Character {
	public class MovingPlatformHandler : MonoBehaviour {
		...
		/// <returns></returns>
		internal float2 HandleMovingPlatforms(CharaCasterDimensions dimensions, float skinWidth, Collider2D collider, float2 velocity, int facingX, LayerMask hMask, LayerMask vMask) {
			...
			if (SupportMovingWalls) {
				...
				Physics2D.BoxCastNonAlloc(
					dimensions.BoxEpicenter(), //Center
				...
				veloCheck[0] = veloCheck.FirstNonSelfHit(collider);
			...
			if (SupportMovingFloors) {
				...
				Physics2D.BoxCastNonAlloc(
					dimensions.BoxEpicenter(), //dimensions.BottomCenter(skinWidth) + new float2(0, skinWidth/2), //Center
